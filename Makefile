...
.PHONY: validate test sync-nav check-site-nav ingest ingest-full analyze pipeline check-analysis help hooks sitemap trends status

help:
	@echo "make validate   - 校验 manifest/候选/hint 决策 + 对账（含 hint-rules 结构）+ 顶栏 + 单测 + analysis_engine --check + 已提交快照契约（与 pre-commit 一致）"
