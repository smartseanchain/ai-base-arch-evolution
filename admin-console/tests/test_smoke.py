...
class TestAdminConsoleSmoke(unittest.TestCase):
    ...
    def test_index_dashboard_html(self) -> None:
        ...
        self.assertIn('content="dark"', body)
        ...
        self.assertIn('aria-label="顶栏：快捷方式、复制链接与刷新数据"', body)
        ...
        self.assertIn("ingest-config（若已配置只读基址）", body)
        ...
        self.assertIn("数据与资源", body)
        self.assertIn("运维与 CI", body)
        self.assertIn("分析与历史", body)
        self.assertIn("API 调试", body)
        ...
        self.assertIn('只读资源片段，点击后在下方探索器拉取对应路径', body)
        self.assertIn("探索器当前请求路径与快捷操作", body)
        ...
        self.assertIn("常用只读 JSON 片段，在下方探索器打开", body)
        ...
        self.assertIn("管道与数据源", body)
        self.assertIn("本地命令链", body)
    ...
    def test_bootstrap_lists_proxy_segments(self) -> None:
        ...
        self.assertRegex(str(data["server_time_utc"]), r"^\d{4}-\d{2}-\d{2}T")
        self.assertIn("admin_accounts_enabled", data)
        self.assertIsInstance(data["admin_accounts_enabled"], bool)
