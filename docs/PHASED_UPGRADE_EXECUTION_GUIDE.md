...
| **改顶栏 / skip-bar 模板** | **`make sync-nav`** → **`make validate`** |
...
**一页合并动线**：[MERGE_AND_RELEASE_CHECKLIST.md](./MERGE_AND_RELEASE_CHECKLIST.md)。**命令表**：[scripts/README.md](../scripts/README.md)。**安装 Git 钩子**（提交前等同 **`make validate`**）：**`make hooks`** 或 `bash scripts/install-git-hooks.sh`。
...
| 4 | 动顶栏模板 → **`make sync-nav`** | **`sync_site_nav --check`** |
| 5 | 扩 **只读 API** | **`make test-readonly-api`**（**`test_readonly*.py`**）；[INTEGRATION](./INTEGRATION_AND_READONLY_API.md) |
| 6 | 动 **admin-console** | **`make test-admin-console`**；[ADMIN_WEB_CONSOLE_ROADMAP](./ADMIN_WEB_CONSOLE_ROADMAP.md) |
...
- 合并前动线：[MERGE_AND_RELEASE_CHECKLIST.md](./MERGE_AND_RELEASE_CHECKLIST.md)  
...
- 技术简版阶段表：[TECH_ARCHITECTURE_AND_UPGRADE_BRIEF.md](./TECH_ARCHITECTURE_AND_UPGRADE_BRIEF.md)  
