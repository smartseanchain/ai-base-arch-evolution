...
全站多处使用 **依据（已可见） / 扩展（机制） / 想象（分叉）**。科学用法是把它们当作**置信度与可复核性的分层**，而不是版式装饰。

1. **依据**：优先引用**已写进站内核或可被第三方核对**的结构与现象（页面正文、法规标题级线索、行业可观察默认栈）。避免把「我觉得」放进依据层。
2. **扩展**：写清**谁影响谁、通过什么机制、延迟大致在哪一段**；若两条链矛盾，应**显式并存**或删掉弱链，不要合成一句顺滑口号。
3. **想象**：必须**至少两条分叉**（或明确写「未知区间」）；分叉之间应能对应**不同前提**（政策窗口、技术采纳率、地缘等），而不是同义反复。

**常见桥接错误**：把「想象」里的生动情景在下一轮复述时悄悄升格为「依据」——纠正办法：回到 [综合推演 · §2 判据](../synthesis.html#criteria) 做一遍对表。

<a id="single-round"></a>

## 3. 单轮推演：建议操作顺序（最小可重复流程）

适合个人或小组**一轮 45–90 分钟**的结构化思考（可与 [EVOLUTION_RUNBOOK](./EVOLUTION_RUNBOOK.md) 双周节奏穿插）：
...
| **把相关当因果** | A 与 B 同向出现即写「A 导致 B」 | §12 表「因果推断」行；改一个前提做反事实口述 |

<a id="multidisciplinary"></a>
...
当站点使用 **manifest / 候选 / 分析快照 / 沉淀** 时，叙事与数据应能对齐，避免「页上写一套、库里另一套」：

- **观测进入站内核**：ingest → 人审 → merge；见 [ARCHITECTURE 数据流](./ARCHITECTURE.md)。
