...
下列与 [综合推演 §12 表](../synthesis.html#methods) **行一一对应**，右两列为本页增补。
...
[§13](../synthesis.html#deep-lens) 的**四穿透**（技术—组织—市场—治理）与**六凝视者**是**元阅读法**，不是独立学科模型：可利用性为 **B**（强制你从多层补变量，防单页过度自信）。与上表**叠加使用**，不单独占「一轮一种」名额。
