...
本文与 **[ARCHITECTURE_ONE_PAGER · 三架构对照](./ARCHITECTURE_ONE_PAGER.md#three-architectures)**、**[ARCHITECTURE_UPGRADE_AND_EXTENSIONS.md](./ARCHITECTURE_UPGRADE_AND_EXTENSIONS.md)**（不变量与阶段 0—3）、**[TECH_ARCHITECTURE_CAPABILITIES.md · §4](./TECH_ARCHITECTURE_CAPABILITIES.md#extend)**（可扩展方向表）对齐，专门回答两件事：**如何把扩展性做到最大**（在不变量之内），以及**平台向下一阶段进化时默认走哪条路**。定性脚手架、非采购清单。**智能化目标表述**已从「单点脚本」升级为 **「六域协同」**（数据 / 管道 / 分析 / 前端 / 运维 / 治理），见专篇 **[INTELLIGENCE_SIX_DOMAINS.md](./INTELLIGENCE_SIX_DOMAINS.md)**。
...
**目标架构（六域协同）**：不把智能化收敛为「某个脚本」，而按 **数据 · 管道 · 分析 · 前端 · 运维 · 治理** 分工与握手；域定义、协同示意、与七类模块/插槽的对照、PR 自检表见 **[INTELLIGENCE_SIX_DOMAINS.md](./INTELLIGENCE_SIX_DOMAINS.md)**。

**可演进的自动化**包括：定时 **ingest**、bot 开候选 PR、**artifact 下载后人工合并**、在已通过 **`make validate`** 前提下用 **`make evolution-fast`** 加速重算快照/沉淀/趋势、递增 **`schema_version`** 与注册表扩展。进化体现在**数据、规则与契约**的迭代，以及 **[§3 四条轨道](#evolution-tracks)** 的分轨推进。
...
**与「分析结论 + AI」分流**：面向**页面叙事**的长草稿走 **`scripts/draft/`**；面向**已生成分析快照之上的解读叠加**（可配置接入模型服务、独立 JSON 产物），设计叙述见 **[AI_ASSISTED_ANALYSIS_LAYER.md](./AI_ASSISTED_ANALYSIS_LAYER.md)**——**不**把非确定性文本并入 **`analysis-snapshot.json` 必填域**，**不**让 LLM 写 manifest。若对标 **GitHub 等开源「舆情 / 热点 / 多源情报」** 产品形态，仅作**参考引用**时的能力映射与侧车边界见 **[REFERENCE_DESIGN_OPINION_MONITORING.md](./REFERENCE_DESIGN_OPINION_MONITORING.md)**。

**合并前推荐动线**：**`make merge-ready`** = **`make validate`** + **`make test-readonly-api`** + **`make test-admin-console`**，见 **[MERGE_AND_RELEASE_CHECKLIST.md](./MERGE_AND_RELEASE_CHECKLIST.md#pre-merge)**。
...
与 [TECH_ARCHITECTURE_CAPABILITIES · §3](./TECH_ARCHITECTURE_CAPABILITIES.md#evolution) **进化三层含义**一致：**进化不必串行**，但每条轨道边界要清；规划时标明本轮主要动哪一条，避免「一条 PR 改全世界」。
...
| **呈现与路由进化** | **MPA** 新枢纽、**SPA** 新路由、**总线**新块。扩展性在于 **registry + partials + `spa-sync`**；**`make validate` 仍以 MPA 为默认真源**。 |
...
- **合并前固定仪式**：**`make merge-ready`** 与发布轻量清单收在 **[MERGE_AND_RELEASE_CHECKLIST.md](./MERGE_AND_RELEASE_CHECKLIST.md)**（**`validate`**、只读 API 与管理端烟测）。  
...
2. **是否新分页/因子？** → **`evolution-registry.json`** → **`make sync-nav`** →（SPA）**`spa/nav.config.json`** + **`make gen-nav-links`**。  
...
- 合并与发布一页清单：**[MERGE_AND_RELEASE_CHECKLIST](./MERGE_AND_RELEASE_CHECKLIST.md)**（含 **[§4 文档索引](./MERGE_AND_RELEASE_CHECKLIST.md#doc-index)**）  
...
- 技术架构整理 + 升级路径（简版）：**[TECH_ARCHITECTURE_AND_UPGRADE_BRIEF](./TECH_ARCHITECTURE_AND_UPGRADE_BRIEF.md)**  
...
- 能力表与未实现方向：**[TECH_ARCHITECTURE_CAPABILITIES · §4](./TECH_ARCHITECTURE_CAPABILITIES.md#extend)**  
